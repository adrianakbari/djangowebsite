...
from .models import Project, ProjectsView
...
class ProjectsListView(ListView):
    ...
    queryset = Project.objects.order_by('-timestamp')[:3]
    paginate_by = 5
...
class ProjectsDetailView(DetailView):
    model = Project
    template_name = 'projects_post.html'
    ...
    def get_object(self):
        ...
        if self.request.user.is_authenticated:
            ProjectsView.objects.get_or_create(
                user=self.request.user,
                project_post=obj
            )
            viewCount = ProjectsView.objects.filter(
                user_id=self.request.user.id, project_post_id=obj.id)[0].view_count
            ProjectsView.objects.filter(
                user_id=self.request.user.id, project_post_id=obj.id).update(view_count=viewCount+1)
        else:
            ProjectsView.objects.get_or_create(
            ...
            viewCount = ProjectsView.objects.filter(
                user_id=2, project_post_id=obj.id)[0].view_count
            ProjectsView.objects.filter(
                user_id=2, project_post_id=obj.id).update(view_count=viewCount+1)
        return obj
...
class ProjectsUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['projects_post_form'] = ProjectsForm(instance=self.object)
        return context
